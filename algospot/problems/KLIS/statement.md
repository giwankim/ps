...
어떤 부분 수열이 \_단조 증가\_할 때 이 부분 수열을 증가 부분 수열 (increasing subsequence) 라고 하며, 이 중 가장 긴 것을 최대 증가 부분 수열 (LIS, longest increasing subsequence) 라고 한다. 예를 들어, `5 20 21 22 8 9 10` 의 최대 증가 부분 수열은 `5 8 9 10` 이다.
