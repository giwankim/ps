...
N개의 국가가 경쟁 중이며, 앞으로 $K$개의 경기가 남았다. 앞선 경기는 반드시 그렇지는 않았지만 남은 경기 모두는 금, 은, 동메달이 서로 다른 국가에 하나씩 주어지는 종목이다. 지금까지 그들이 획득한 메달의 수가 주어질 때, 앞으로 남은 $K$ 경기의 결과에 따른 최선의 등수를 두 방법 각각에 대해 구하는 프로그램을 작성하라.
