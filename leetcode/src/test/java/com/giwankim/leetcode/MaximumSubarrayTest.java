...
class MaximumSubarrayTest {
  ...
  // Step 2: single negative element — guards against an empty-subarray fallback that would return 0.
  @Test
  ...
  // Step 6: LC Example 3 — [5, 4, -1, 7, 8] → 23. Interior negative does not break the running sum, so the whole array remains optimal.
  @Test
  ...
  // Step 11: a large intermediate negative breaks the running sum so the algorithm must drop the prefix and restart.
  @Test
  ...
  // Step 13: maximum length per the LC constraint nums.length <= 10^5 — exercises linear-time scaling at the upper bound.
  @Test
