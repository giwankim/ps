...
public class ConstructQuadTree {
   ...
   *     grid.length}.
   *     <p><b>Time:</b> the recurrence {@code T(n) = 4·T(n/2) + O(1)} captures four recursive calls
   *     on sub-grids of side {@code n/2} plus constant work per call (midpoint arithmetic, the
   *     four-way leaf-and-value check, and one {@link Node} allocation). Applying the master
   *     theorem with {@code a = 4}, {@code b = 2}, {@code f(n) = O(1)}: {@code n^log_b(a) = n²}
   *     strictly dominates {@code f(n)}, so case 1 gives {@code T(n) = Θ(n²)}.
   *     <p><b>Space:</b> only one of the four child calls is on the stack at a time, so the
   *     auxiliary-stack recurrence is {@code S(n) = S(n/2) + O(1) = O(log n)}. The output quadtree
   *     is not counted.
   */
